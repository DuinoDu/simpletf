...
* C++ source files:
  * `include/simpletf/simpletf.hpp` is the main
    C++ header that declares the interface of your library.
  * `src/simpletf.cpp` is the main file that implements this library.
  * `app/simpletf_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/simpletf_t.cpp` contains the unit tests for the library.
  ...
  * The `python/simpletf` directory contains a Python
    package for the project. It contains a compiled Python module `_simpletf`
    that CMake generates from the Pybind11 source file `python/simpletf/_simpletf.cpp`. Additionally, it contains pure Python sources (e.g. `__init__.py`) that
